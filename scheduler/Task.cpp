...
namespace cs3100
{
  Job::Job(float t, int devices, int pages) : creationTime(t), cur(0), tasks()
  {
    ...
    // start with CPU
    tasks.push_back(getTask(Task::Type::CPU, bindType, devices-1, pages-1));
    // jobs might be cpu or io bound, we choose evenly between the two options
    auto numTasks = getInt(5, 10);
    for (int i = 0; i < numTasks; ++i)
    {
      tasks.push_back(getTask(getType(), bindType, devices-1, pages-1));
    }
    // end with CPU
    tasks.push_back(getTask(Task::Type::CPU, bindType, devices-1, pages-1));
  }

  float Job::totalJobDuration()
  {
    float total = 0.0;
    for(auto task : tasks)
    {
  ...
  float Job::totalCPUDuration()
  {
    float total = 0.0;
    for(auto task : tasks)
    {
      if(task.type == cs3100::Task::Type::CPU)
      {
        total += task.duration;
      }
    }
